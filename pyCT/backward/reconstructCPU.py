...
def reconstructConeBeamCPU(reconstruction_array, transformation, sinogram_array, nx, ny, nz, nu, nv, na, su, sv, du, dv, ou, ov, s2d):
    pad = 1
    sinogram_array = np.pad(sinogram_array, [(0,0),(pad,pad),(pad,pad)])
    nu += (2*pad -1)
    nv += (2*pad -1)
        ...
        for x in range(nx):
            for y in range(ny):
                for z in range(nz):
                    ...
                    u = (u / w * -s2d + su/2 - ou)/du + pad
                    v = (v / w * -s2d + sv/2 - ov)/dv + pad
                    if 0 < u < nu and 0 < v < nv:
                        u0, v0 = int(u), int(v)
                        u1, v1 = u0 + 1, v0 + 1
                        interp = (v1-v) * ((u1-u)*sinogram_array[a,v0,u0]+(u-u0)*sinogram_array[a,v0,u1]) + (v-v0) * ((u1-u)*sinogram_array[a,v1,u0]+(u-u0)*sinogram_array[a,v1,u1])
