...
def projectConeBeamCPU(detector_array, transformation, object_array, nx, ny, nz, nu, nv, nw, na, su, sv, ou, ov, s2d, near, far):
    pad = 1
    object_array = np.pad(object_array, pad)
    nx += (2*pad -1)
    ny += (2*pad -1)
    nz += (2*pad -1)    
    U, V = np.meshgrid(np.linspace(-su/2+(su/nu)/2, su/2-(su/nu)/2, nu) + ou, np.linspace(-sv/2+(sv/nv)/2, sv/2-(sv/nv)/2, nv) + ov)
